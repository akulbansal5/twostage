...
# 
#BeAng_gurobi: Same as BeAng final but changes the solver from cplex to Gurobi, and does not use callbacks.
        ...
        for id in sub_idxs:
            ...
            #it not then add integer L-shaped cut
                ...
                if sub.curr_obj > tVec[id] + tolerance:
                    ...
                    # coeff              = sub.curr_obj - global_model.global_lower_bd
                    coeff              = sub.curr_obj - sub.lower_bound
                    rhs                = -coeff*ones + sub.curr_obj
                    ...
                    master.addConstr(intL_expr >= rhs, name = f"lap_{count}_{id}")
                    cut_store[id].append(f"lap_{count}_{id}")
                    constr_sol_store[f'lap_{count}_{id}'] = masterObj.curr_sol
                    constr_subobj_store[f'lap_{count}_{id}'] = sub.curr_obj
                    gfc_obtain_time += time.time() - time_counter
                # elif tVec[id] > subCosts[id]:
                #     print(count, id, tVec[id], subCosts[id])
                #     #check the binding constraints
                #     for c_name in cut_store[id]:
                #         c_obj = master.getConstrByName(c_name)
                #         if abs(c_obj.Slack) < 1e-6:
                #             print(f"Constraint Name: {c_name}")
                #             print(f"State vector at time of adding: {constr_sol_store[c_name]}")
                #             print(f"State vector now: {masterObj.curr_sol}")
                #             print(f"Sub objective at time of adding: {constr_subobj_store[c_name]}")
                #             master.write(f"master_{count}.lp")
                #             break
                    
                #     raise ValueError("tSolution is UB of what is obtained by directly solving the sub")
...
global_runs_BeAng(fileofobjects, gade_iter = 200, total_time = 600, gade_tol = 1e-4, ben_tol = 1e-3, ben_iter = 10000, ben_time = 3600, csvname = "benders_angulo_output_Apr1_24.csv", isEqual = True, addCopy = True, pFlag = True, earlyGap = 1e-1, earlypFlag = False)
