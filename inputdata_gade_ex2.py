...
def def_sub_gade(rhs_const):
    ...
    sub_varTypes = ['I'] + ['I']*4 + ['I'] + ['B']*2
    
    lbs = [-cpx.infinity] + [0]*4 +[0] +  [0]*2
    ubs = [cpx.infinity] + [5]*4 + [cpx.infinity] + [1]*2
    
    objs = [1] + [0]*7
    
    
    sub_prob.variables.add(obj = objs, types = sub_varTypes, lb = lbs, ub = ubs, names = sub_varNames)
    ...
    rhs_col = [0] + [0,0] + [5]*4                                   #note we need to update this for each scenario
...
def subproblem_fixed_recourse_gade(rhs_const):
    ...
    ubs = [cpx.infinity] + [5]*4 + [cpx.infinity]
    objs = [1] + [0]*5
    
    sub_prob.variables.add(obj = objs, lb = lbs, ub = ubs, names = sub_varNames)
    ...
    #note 5 is the upperbound that we have over here
    rhs_col = [0] + [0,0] + [5]*4                                   #note we need to update this for each scenario
    row_names = [f'c_{i}' for i in range(2, 2+7)]                   #name of constraints 
...
sub0 = Subproblem(0)
sub0.rhs_const = np.array([0, 10, 4] + [5]*4)
...
sub1 = Subproblem(1)
sub1.rhs_const    = np.array([0, 13, 8] + [5]*4)
...
sub1.cpx_def_copy = def_sub_gade(sub1.rhs_const)
